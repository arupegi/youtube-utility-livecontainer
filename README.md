...
低速通信テストの画面録画をもとに、回線が遅いときの長時間停止を改善しました。

### ページ全体のreloadを廃止
Content Rule変更時にYouTubeページ全体をreloadしないよう変更しました。
低速回線でも現在のプレイヤーとページ状態を維持します。

### 音声のみモード
googlevideoの映像セグメントを完全blockする方式は、
低速回線でYouTubeのMSEプレイヤーをバッファリング状態に固定することがあるため削除しました。

代わりに:
- YouTube内部プレイヤーへ最低品質 `tiny` を要求
- 映像はほぼ透明化
- 音声とプレイヤー状態は維持

純粋な映像ゼロより通信量は少し増えますが、安定性を優先しています。

### シーク復帰
低速時のシーク後に停止したままになるケースへ、
再生中だった場合だけ700ms後に再生復帰を試す処理を追加しました。

### UI
ページ読み込み中は小さなProgressViewのみ表示し、操作画面全体は塞ぎません。
...
### 目的
- 画像系通信をできるだけ減らす
- 一覧画面では **タイトル + チャンネル名** を中心に見る
- コメント / チャット / 関連 / Shorts / Mix / シークプレビューなどを極力抑える

### 主な挙動
- 一覧はタイトルとチャンネル名を優先した最小表示
- サムネイル画像・チャンネルアイコンなどを極力非表示
- コメント欄 / ライブチャット / 関連動画を非表示
- Shorts / Mixを非表示
- シークプレビューを非表示
- 低速回線向けに低品質再生を優先

### 補足
YouTubeの一覧APIはタイトル・チャンネル名・視聴回数・投稿日などを
同じレスポンスで返すことが多いため、それらの**文字データだけを通信レベルで
個別に切ることは困難**です。

そのため v0.17 の最小通信モードでは、**通信量の大きい画像系と周辺機能を
優先して削減しつつ、画面表示はタイトル + チャンネル名中心に整理する**
構成にしています。


## v0.18 最小通信モード ハイブリッド方式

最小通信モードの再生方式をハイブリッド化しました。

### 節約モード
再生が安定している間は、video-onlyのGoogleVideoセグメントを
Content Ruleで強く抑えます。

### 安定モード
`waiting` / `stalled` が約2秒以上続いた場合:

1. video-onlyセグメントの強制blockを解除
2. YouTube内部プレイヤーへ `tiny` 品質を要求
3. 再生復帰を試行

ページ全体のreloadは行いません。

### 自動再試行
安定モードで約12秒間連続して再生できた場合、
再び節約モードへの移行を試します。

### シーク
シーク開始時は一度安定モードへ戻します。
シーク後に再生が安定すれば、再び節約モードを試します。
